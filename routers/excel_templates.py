...
BUILTIN_ALIASES = {
    "real_name":     ["姓名", "户主姓名", "农户姓名", "名字", "姓 名", "户主名字", "申请人姓名"],
    "id_card":       ["身份证号", "身份证", "证件号码", "居民身份证号", "身份证号码", "证件号", "ID号"],
    "phone":         ["手机号", "电话", "联系电话", "手机", "联系方式", "手机号码", "电话号码"],
    "bank_card":     ["银行卡号", "卡号", "账号", "银行账号", "打款账号", "收款账号", "银行卡"],
    "bank_name":     ["开户行", "银行", "开户银行", "开户网点", "银行名称"],
    "village_name":  ["所在村", "村名", "村庄", "村", "行政村", "所在行政村","社区"],
    "group_no":      ["所在组", "组号", "组", "村组", "小组", "村民小组"],
    "land_area":     ["实际种植面积", "面积", "承包面积", "土地面积（亩）", "耕地面积", "亩数", "承包面积(亩)"],
    "address":       ["地址", "住址", "家庭住址", "详细地址"],
    "farmer_status": ["状态", "农户状态", "在册状态"],
    "gender":        ["性别", "性 别"],
    ...
    "trust_area":    ["代耕代种面积", "代种面积", "托管面积", "代耕面积", "代耕代种面积(亩)", "代种面积(亩)"],
}
    ...
    if business_type == "SUBSIDY":
        ...
        for row in mapped_rows:
            ...
            hh = FamilyHousehold(household_code=f'HH{fp.id:05d}', household_name=f'{nm}户',
                                 head_farmer_id=fp.id, village_group_id=vg.id, status=1)
