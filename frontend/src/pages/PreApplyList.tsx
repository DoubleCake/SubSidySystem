...
export default function PreApplyList({
  ...
  const handleExport = async () => {
    try {
      const res = await api.exportApplications(subsidyType.id)
      ...
      XLSX.utils.book_append_sheet(wb, ws, '预申请列表')
      XLSX.writeFile(wb, `预申请列表_${subsidyType.subsidy_name}_${new Date().toISOString().slice(0, 10)}.xlsx`)
    } catch (e) {
      ...
      <div className="bg-white border border-border rounded-card overflow-x-auto shadow-card">
        ...
        <div className="px-4 py-3 border-b border-border bg-warm/10 flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1 flex-1 min-w-[200px] max-w-[300px]">
            ...
            <button onClick={() => onSearch ? onSearch() : setPage(1)} className="px-2 py-1 text-xs bg-primary rounded-btn hover:bg-primary/90 border border-border px-2 py-1 rounded">搜索</button>
          </div>
