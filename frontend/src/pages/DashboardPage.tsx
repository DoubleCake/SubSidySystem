  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-5">
        ...
        <button onClick={() => {
          setLoading(true)
          Promise.all([api.getYearCompare(year), api.getSummaryByVillage(year), api.getSubsidyTypesWithStats(year)])
            .then(([c,v,s])=>{ setCompare(c); setByVillage(v); setStats(s as StatsType[]) })
            .finally(()=>setLoading(false))
        }} className="px-3 py-2 text-sm border border-stone-200 rounded-lg bg-white text-stone-500 hover:bg-stone-50">
          刷新
        </button>
        {loading && <span className="text-xs text-stone-400">加载中…</span>}
      </div>
