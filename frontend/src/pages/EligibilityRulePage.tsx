  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-bold text-stone-800">资格规则配置</h3>
          <p className="text-xs text-stone-400 mt-0.5">「{subsidyName}」的发放资格条件，导入时自动校验</p>
        </div>
      ...
      {rules.length === 0 && !loading && (
        <div className="mb-4">
          <p className="text-xs text-stone-400 mb-2">📌 快速添加常用规则：</p>
      ...
      {/* 规则列表 */}
      {loading && <div className="py-8 text-center text-stone-300 text-sm">加载中…</div>}
      {!loading && rules.length === 0 && (
        <div className="py-10 text-center bg-stone-50 border border-dashed border-stone-200 rounded-xl">
          <div className="text-3xl mb-2">📋</div>
          <p className="text-sm text-stone-400 mb-1">暂无资格规则</p>
          <p className="text-xs text-stone-300">不配置规则时，所有农户均可通过资格检查</p>
        </div>
      )}
      <div className="space-y-2">
        {rules.map(r => (
          <div key={r.id} className="bg-white border border-stone-200 rounded-xl p-4 hover:border-stone-300 transition-colors">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="font-semibold text-stone-800 text-sm">{r.rule_name}</span>
                  <Tag label="启用" color="green" />
                  {!r.can_combine_with_others && <Tag label="不可叠加" color="red" />}
                </div>
                {r.rule_desc && <p className="text-xs text-stone-400 mb-2">{r.rule_desc}</p>}
                <div className="flex flex-wrap gap-1.5">
                  {ruleDesc(r).map((line, i) => (
                    <span key={i} className="text-xs bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-0.5 rounded">
                      {line}
                    </span>
                  ))}
                  {ruleDesc(r).length === 0 && (
                    <span className="text-xs text-stone-300">（无具体条件限制）</span>
                  )}
                </div>
              </div>
              <div className="flex gap-1.5 shrink-0">
                <button onClick={() => openEdit(r)}
                  className="text-xs border border-stone-200 text-stone-500 px-2.5 py-1 rounded hover:border-stone-300">
                  编辑
                </button>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="block text-xs text-stone-400 mb-1">规则名称 *</label>
            ...
            <div className="col-span-2">
              <label className="block text-xs text-stone-400 mb-1">规则说明（给操作员看）</label>
          ...
          <div className="border-t border-stone-100 pt-3">
            <p className="text-xs font-semibold text-stone-500 mb-2">人员条件</p>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-xs text-stone-400 mb-1">要求农户状态</label>
              ...
              <div>
                <label className="block text-xs text-stone-400 mb-1">最小年龄（岁）</label>
                <input type="number" value={form.require_age_min ?? ''} onChange={e => sf('require_age_min', e.target.value ? Number(e.target.value) : null)}
                  placeholder="不限" className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-400" />
              </div>
              <div>
                <label className="block text-xs text-stone-400 mb-1">最大年龄（岁）</label>
          ...
          <div className="border-t border-stone-100 pt-3">
            <p className="text-xs font-semibold text-stone-500 mb-2">土地条件</p>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-xs text-stone-400 mb-1">地块类型</label>
              ...
              <div>
                <label className="block text-xs text-stone-400 mb-1">最小面积（亩）</label>
              ...
              <div>
                <label className="block text-xs text-stone-400 mb-1">最大面积（亩）</label>
          ...
          <div className="border-t border-stone-100 pt-3">
            <p className="text-xs font-semibold text-stone-500 mb-2">叠加规则</p>
            <label className="flex items-center gap-1.5 text-sm text-stone-600 cursor-pointer">
