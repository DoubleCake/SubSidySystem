...
const ACTION_LABEL: Record<string, { label: string; color: string }> = {
  create:      { label: '新建家庭户', color: 'bg-emerald-100 text-emerald-700' },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-stone-800">家庭户批量导入</h1>
        ...
        {step > 1 && (
          <button onClick={resetAll}
            className="text-sm text-stone-400 hover:text-stone-600 border border-stone-200 rounded-lg px-3 py-1.5">
            重新上传
          </button>
        )}
      </div>
      ...
      {/* 步骤条 */}
      <div className="flex items-center gap-0 bg-white rounded-xl border border-stone-200 p-4">
        {STEPS.map((s, i) => (
          <div key={i} className="flex items-center flex-1">
            <div className={`flex items-center gap-2 ${i + 1 === step ? 'text-emerald-700 font-semibold' : i + 1 < step ? 'text-emerald-500' : 'text-stone-300'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold
                ${i + 1 === step ? 'bg-emerald-600 text-white' : i + 1 < step ? 'bg-emerald-100 text-emerald-600' : 'bg-stone-100 text-stone-400'}`}>
                {i + 1 < step ? '✓' : i + 1}
              </div>
              <span className="text-sm whitespace-nowrap">{s}</span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`flex-1 h-px mx-3 ${i + 1 < step ? 'bg-emerald-200' : 'bg-stone-100'}`} />
            )}
          </div>
        ))}
      </div>
      ...
      {/* ── Step 1: 上传文件 ── */}
      {step === 1 && (
        <div className="bg-white rounded-xl border border-stone-200 p-8 flex flex-col items-center gap-4">
          <div className="text-4xl">📂</div>
          <p className="text-stone-600 text-sm">上传包含家庭户信息的 Excel 文件（.xlsx / .xls）</p>
          <p className="text-stone-400 text-xs">必须包含：姓名、身份证号、家庭住址；建议包含：户主关系</p>
          <label className="cursor-pointer bg-emerald-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-emerald-700 transition-colors">
            选择文件
      ...
      {step === 2 && (
        <div className="bg-white rounded-xl border border-stone-200 p-6 space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-stone-800">列映射配置</h2>
            <span className="text-xs text-stone-400">已读取 {rawRows.length} 行数据，{headers.length} 列</span>
          </div>
          ...
          {/* 数据预览 */}
          <div>
            <p className="text-xs text-stone-400 mb-2">前 3 行预览：</p>
            <div className="overflow-x-auto border border-stone-100 rounded-lg">
              <table className="text-xs w-full">
                <thead className="bg-stone-50">
                  <tr>{headers.map(h => <th key={h} className="px-3 py-2 text-left text-stone-500 font-normal whitespace-nowrap">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {rawRows.slice(0, 3).map((r, i) => (
                    <tr key={i} className="border-t border-stone-100">
                      {headers.map(h => <td key={h} className="px-3 py-1.5 text-stone-600 whitespace-nowrap">{r[h]}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex justify-end">
            <button onClick={handlePreview} disabled={loading}
              className="bg-emerald-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-emerald-700 disabled:opacity-50 transition-colors">
              {loading ? '分析中…' : '预览导入结果 →'}
            </button>
          </div>
        </div>
      ...
      {step === 3 && preview && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-5 gap-3">
            {[
              { label: '总行数', value: preview.summary.total_rows, color: 'text-stone-700' },
              { label: '家庭户组', value: preview.summary.total_groups, color: 'text-stone-700' },
              { label: '新建', value: preview.summary.new_households, color: 'text-emerald-600' },
            ...
            <div className="px-4 py-3 border-b border-stone-100 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setExpandedGroups(new Set(preview.groups.map((_, i) => i)))}
                  className="text-xs text-emerald-600 hover:underline">全部展开</button>
                <button onClick={() => setExpandedGroups(new Set())}
                  className="text-xs text-stone-400 hover:underline">全部折叠</button>
              </div>
            </div>
                ...
                return (
                  <div key={i} className={g.has_errors ? 'bg-red-50' : ''}>
                    ...
                    <div
                      ...
                      <span className="text-stone-400 text-xs w-4">{expanded ? '▾' : '▸'}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ac.color}`}>{ac.label}</span>
                      <span className="text-sm text-stone-700 flex-1 truncate">{g.address}</span>
                      <span className="text-xs text-stone-400">{g.member_count} 人</span>
                      <span className="text-xs text-stone-500">户主：{g.head_name}</span>
                    ...
                    {expanded && (
                      <div className="px-10 pb-3 space-y-3">
                        ...
                        {g.matched_hh_info.length > 0 && (
                          <div>
                            <p className="text-xs text-stone-400 mb-1">匹配到的已有家庭户：</p>
                            ...
                            {g.total_area_after_merge != null && (
                              <p className="text-xs text-stone-400 mt-1">合并后承包面积（均值）：<b className="text-stone-700">{g.total_area_after_merge} 亩</b></p>
                            )}
                          </div>
                        )}
                        ...
                        <table className="text-xs w-full">
                          ...
                          <tbody>
                            {g.members.map((m, mi) => (
                              <tr key={mi} className={m.has_errors ? 'text-red-500' : 'text-stone-600'}>
                                <td className="py-0.5">{m.real_name}</td>
                                <td className="py-0.5 font-mono">{m.id_card.slice(0, 6)}****{m.id_card.slice(-4)}</td>
                                <td className="py-0.5">{m.is_head ? <span className="text-emerald-600 font-medium">户主</span> : '成员'}</td>
                                <td className="py-0.5">{m.in_db ? <span className="text-blue-500">已存在（跳过）</span> : <span className="text-emerald-500">新增</span>}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
          ...
          <div className="flex justify-between items-center">
            <button onClick={() => setStep(2)}
              className="text-sm text-stone-400 border border-stone-200 rounded-lg px-4 py-2 hover:text-stone-600">
              ← 返回列映射
            </button>
            <button onClick={handleExecute} disabled={loading}
              className="bg-emerald-600 text-white px-8 py-2 rounded-lg text-sm hover:bg-emerald-700 disabled:opacity-50 transition-colors font-medium">
              {loading ? '导入中…' : '确认导入'}
            </button>
          </div>
        </div>
      ...
      {step === 4 && result && (
        <div className="bg-white rounded-xl border border-stone-200 p-8 space-y-6">
          <div className="text-center">
            ...
            <h2 className="font-bold text-stone-800 text-lg">导入完成</h2>
          </div>

          <div className="grid grid-cols-4 gap-4">
            {[
              { label: '新建家庭户', value: result.created_households, color: 'text-emerald-600' },
              { label: '合并/更新户', value: result.merged_households, color: 'text-blue-600' },
              { label: '新增成员', value: result.created_farmers, color: 'text-emerald-600' },
              { label: '跳过成员', value: result.skipped_farmers, color: 'text-stone-400' },
            ].map(({ label, value, color }) => (
              <div key={label} className="text-center border border-stone-100 rounded-xl p-4">
          ...
          <div className="flex justify-center gap-3">
            <button onClick={resetAll}
              className="border border-stone-200 text-stone-600 px-6 py-2 rounded-lg text-sm hover:bg-stone-50">
              继续导入
            </button>
            <a href="/farmers"
